...
const nameSchema = z.string({ error: requiredError }).min(1, '必須です').max(255);   // VARCHAR(255) NOT NULL
...
// レシピ本体（repos_information）と子テーブルの入力。POST /api/repos と PATCH /api/repos/:id の
...
const recipeSchema = z.preprocess(
    // title は name でも送れる（README の API 仕様）ので、先に title に寄せる
    (body) => (
        body !== null && typeof body === 'object' && body.title === undefined && body.name !== undefined
...
const commitParamsSchema = repoParamsSchema.extend({
